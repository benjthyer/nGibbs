...
# OCt 16: Build customizable network
"""
class TunableModel(nn.Module):
    def __init__(self, encoderLayerUp = 2, encoderLayerDown = 1, middleLayerUp = 2, middleLayerDown = 1, 
                regularization = 'none', activation = nn.LeakyReLU(0.05)):
        #Let regularization be a string in ['none', 'batchnorm', 'dropout<#>'], and activation a complete nn.ActivationFunction
        regname = pull_letter(regularization).lower()
        assert regname in ['none', 'batchnorm', 'dropout'], f"regularization arg must be one of: ['none', 'batchnorm', 'dropout<#>']"
        assert encoderLayerUp >= encoderLayerDown, "EncoderLayerUp must be greater than or equal to encoder Layer Down"
        assert middleLayerUp >= middleLayerDown, "middleLayerUp must be greater than or equal to middleLayerDown"
        ...
        if regname == 'dropout':
            fraction = pull_number(regularization)
            assert fraction > 0 and fraction < 1, f"number in dropout<#> must be between 0 and 1."
            def regularization_activation(unusedArg):
                return activation, nn.Dropout(fraction)

        elif regname == 'batchnorm':
            def regularization_activation(neurons):
                return nn.BatchNorm1d(neurons), activation
        ...
        # Build encoder
        encoderlayers = [nn.Linear(input_dim, 64, bias=False) if regname == 'batchnorm' else nn.Linear(input_dim, 64), regularization_activation(64)]
        prevNeuron = 64

        for i in range(encoderLayerUp):
            encoderlayers.append(nn.Linear(prevNeuron, prevNeuron*2, bias=False) if regname == 'batchnorm' else nn.Linear(prevNeuron, prevNeuron*2, 64))
            encoderlayers += [regularization_activation(prevNeuron*2)]
            prevNeuron *= 2
        for i in range(encoderLayerDown):
            encoderlayers.append(nn.Linear(prevNeuron, prevNeuron/2, bias=False) if regname == 'batchnorm' else nn.Linear(prevNeuron, prevNeuron/2, 64))
            encoderlayers += [regularization_activation(prevNeuron/2)]
            prevNeuron /= 2

        self.encoder = nn.Sequential(*encoderlayers) # Shared encoder
        
        # Independent binary classification heads (1 per phase)
        self.sat_heads = nn.ModuleList([
            nn.Sequential(
                nn.Linear(prevNeuron, 16, bias=False) if regname == 'batchnorm' else nn.Linear(prevNeuron, 16),
                regularization_activation(16),
                nn.Linear(16, 1)
        ...
        # Build Middle Layers
        if middleLayerUp >= 1:
            middlelayers = [nn.Linear(prevNeuron + self.n_phases, prevNeuron*2, bias=False) if regname == 'batchnorm' else nn.Linear(prevNeuron + self.n_phases, prevNeuron*2), 
                            regularization_activation(prevNeuron*2)]
            prevNeuron *= 2
            for i in range(chemLayerUp-1):
                middlelayers.append(nn.Linear(prevNeuron, prevNeuron*2, bias=False) if regname == 'batchnorm' else nn.Linear(prevNeuron, prevNeuron*2, 64))
                middlelayers += [regularization_activation(prevNeuron*2)]
                prevNeuron *= 2

            for i in range(middleLayerDown):
                middlelayers.append(nn.Linear(prevNeuron, prevNeuron/2, bias=False) if regname == 'batchnorm' else nn.Linear(prevNeuron, prevNeuron/2, 64))
                middlelayers += [regularization_activation(prevNeuron/2)]
                prevNeuron /= 2

            self.middleBrain = nn.Sequential(*middlelayers)
            middleOut = prevNeuron 
            
        else:
            self.middleBrain = None
            middleOut = prevNeuron + self.n_phases

        self.mole_head = nn.Sequential(
            nn.Linear(middleOut, 64, bias=False) if regname == 'batchnorm' else nn.Linear(middleOut, 64),
            regularization_activation(64)
            
            nn.Linear(64, self.n_phases), # Output to be put through softplus function
            nn.Softplus()
        )


        class PhaseHead(nn.Module): # Make a class to handle masking before softmax of chemheads
            def __init__(self, n_components, inputDim=middleOut):
                super().__init__()
                self.fc = nn.Linear(inputDim, n_components)
        ...
        for i, (label, inds) in enumerate(label_indices.items()):
            ...
            if n_components > 1:
                ...
                self.comp_mappingsL = self.comp_mappingsL + np.repeat(j, n_components).tolist()
                self.chem_list.append(PhaseHead(n_components=n_components, inputDim=))
                j += 1
...
class MidLevelNetwork(TunableModel):
    ...
    #Wrapper with more functionality
    
    def __init__(self, encoderLayerUp = 2, encoderLayerDown = 1, middleLayerUp = 2, middleLayerDown = 1, 
                regularization = 'none', activation = nn.LeakyReLU(0.05)):
        super.__init__(encoderLayerUp, encoderLayerDown, middleLayerUp, middleLayerDown, regularization, activation)
        ...
        """
# --- BEGIN: TunableModel corrected implementation ---
class TunableModel(nn.Module):
    def __init__(self,
                 encoderLayerUp=1,
                 encoderLayerDown=0,
        ...
        for _ in range(self.n_phases):
            head_layers = [nn.Linear(self._encoder_output_dim, 16)]
    ...
    def finish_build(self):
        ...
        This separation allows use of globals that may be set after __init__ in your code.
        """
...
# --- END: TunableModel ---
# --- BEGIN: MidLevelNetwork corrected subclass ---
class MidLevelNetwork(TunableModel):
    def __init__(self, encoderLayerUp=0, encoderLayerDown=0,
        ...
        #self.register_buffer('pure_binaries_bool', pure_bool)
        ...
        #elf.fixed_phaseToCompMap = torch.tensor(fixed_phaseToCompMap, dtype = torch.float, device = 'cuda')
        ...
        #self.pure_binaries_bool = torch.ones(i+1).to(bool)
        #self.pure_binaries_bool[torch.tensor(comp_binariesL, dtype = torch.bool)] = False
        ...
        # NOTE: you must register other mapping buffers like boolTransCompToOx, phaseToCompMap, etc. in your code
        # they are left as-is assuming you will set them in the outer scope and register them here similarly.
    ...
    def forward_binaries(self, x):
        ...
        #return self.sat_head(latent)
    
    """def forward_chemistry(self, x, binaries):
        # Retiring 10/16/25, moles and chemistry now more closely related. Given binaries, outputs intensive phase chemistries for training chem heads
        # Encode features
        latent = self.encoder(x)
...
class NN_MELTS():
    ...
    def __init__(self, Model, min_tensor=min_tensor, range_tensor=range_tensor, cuda = False):#ModelCr, ModelNoCr, cuda = False):#'components_resampled_normalization_stats.txt'):
    ...
    def forward_binary(self, features, normalize = True, WtPercent = True):
        """Outputs probability of phase saturation. Still does not explicitly prevent impossible phases (e.g. apatite when no PO4 present)"""
        if normalize: 
  ...
  # (B, C)
    ...
    def polish_masses(self, phaseMoles, reconBulk, componentMoles, phaseProportions, features, optimize_masses = True, output_componentMoles= False, protect_opx = False, comp_table_out = 'oxides'):
