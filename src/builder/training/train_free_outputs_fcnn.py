...
1) features -> free outputs
2) [features, phase moles, intensive phase labels] -> free outputs
...
from nMELTS.engine.NN import VariableGeometryFCNNRegressor
...
def main() -> None:
    ...
    parser.add_argument("--skip-1", action="store_true", help="Skip training the features-only model")
    parser.add_argument(
        "--out-dir",
        type=Path,
    ...
    if not args.skip_1:
        ...
        print("\nTraining model 1/2: features -> free outputs")
        best_model1_path = args.out_dir / f"best_features_only_{args.bundle_stem.name}.pt"
    ...
    print("\nTraining model 2/2: [features, moles, labels] -> free outputs")
    ...
    # Load best checkpoints for evaluation if available
        ...
        metrics["features_only"]["train"] = _evaluate_denorm(
            model=model_features_only,
            x=x1_train,
        ...
        metrics["features_only"]["test"] = _evaluate_denorm(
            model=model_features_only,
            x=x1_test,
    ...
    if not args.skip_1:
        metrics["features_only"]["valid"] = _evaluate_denorm(
            model=model_features_only,
            x=x1_valid,
    ...
    model1_path = args.out_dir / f"free_outputs_features_only_{bundle_basename}.pt" if not args.skip_1 else "Not Trained!"
    ...
    if not args.skip_1:
        _save_checkpoint(
            ...
            y_min=y_min,
            y_range=y_range,
            input_kind="features_only",
            metrics=metrics["features_only"],
        ...
        json.dump(
            {
                ...
                "normalizers": {
                    "features_only_input_min_range": _normalizer_pairs(x1_min, x1_range).tolist(),
                    "extended_input_min_range": _normalizer_pairs(x2_min, x2_range).tolist(),
                    "output_min_range": _normalizer_pairs(y_min, y_range).tolist(),
                },
                "metrics": metrics,
                "model_paths": {
                    "features_only": str(model1_path),
