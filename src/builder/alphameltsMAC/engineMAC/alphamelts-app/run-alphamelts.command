...
my ($version, $newversion, $path, $run_path, $batch, $auto, %switch);
...
until (@argv2) {
	...
	warn ("\n usage: run_alphamelts2\.command \n\n".
	      ...
	      " [-c column_file]   run 'column_pick.command column_file' for each file in column_files before moving files \n".
	      " [-o output_file]   filename for output of column_pick.command (default alphaMELTS_tbl.txt) \n".
	      ...
	      "                    (or use with '-p' or '-c' to move or run column_pick on previously generated files) \n\n". 
	      " This is run_alphamelts2.command $version; use it with alphamelts $version or updates $version.X.\n\n");
	next;
    }
    elsif ($newversion) {
    ...
    $run = '';
    $program = 'timeout 60 alphamelts2';
	...
	unless (open (LOGFILE, '>', 'logfile.txt')) {
	    ...
	    warn "RUN-ALPHAMELTS.COMMAND WARNING: Cannot open (temporary) log_file logfile.txt ($!)\n";
	    next;
	}
    }
    if ($in_file) {
