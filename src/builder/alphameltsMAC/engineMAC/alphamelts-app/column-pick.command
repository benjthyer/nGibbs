  ...
  FOREACH: foreach my $colfile (@argv2) {
	  ...
	  if(/^Name/) {
		...
		if ($table && !$tablename) {
		    ...
		    map {$values{$labels[$i++]} = $_} split /$indelim+/; # one value for each matched column label
			...
			$lines[1] .= (join "_$tablename$outdelim", @columns)."_$tablename$outdelim";
		    }
		    elsif ($header eq 'excel') { # table name already printed
			$lines[1] .= (join $outdelim, @columns).$outdelim;
		    }
		    elsif (!@index) { # Phase table for System ot other full table
			$lines[1] .= (join $outdelim, @columns).$outdelim;
		    }
