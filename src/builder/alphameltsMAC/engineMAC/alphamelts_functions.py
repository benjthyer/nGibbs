...
alphaMELTSLocation = os.path.join(Path(__file__).parent.absolute(), 'alphamelts-app')
...
def forward_ensemble(input_array, keys, batchname, only_phases=None, end=0, EnsembleLocation=EnsembleLocation,
                     ...
                     compression=False, delta=-3, suppress_with_batch=True):
    """
...
def import_MELTS_components(EnsembleLocation, batchname, indexer, fO2Arr=None, 
    ...
    # Normalize fluid rows to sum 1 molar
    ...
    assert unique_faults < (folders * 0.95), \
        f"FAILURE THRESHOLD EXCEEDED: {unique_faults} out of {folders} simulations failed " \
        f"(failure rate: {failure_rate*100:.2f}%). More than 95% failure rate is unacceptable."
