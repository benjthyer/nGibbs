	...
	save x1a
        logical trace821_opened, trace822_opened
        save trace821_opened, trace822_opened
        data trace821_opened /.false./
        data trace822_opened /.false./
...
c         write(31,'(a39,i5,12f12.5)') 'WARNING: Volume encountered instability',ispec,Pi,Ti,vol,Vo,x1
	 ...
	 spinod(ispec) = .true.
        ...
        volnl = vol
	x1a(ispec) = vol
C       Phase-entry checkpoint: resolved volume before thermodynamics
        if (.not. trace821_opened) then
         open(821,file='fort.821',status='unknown',form='formatted')
         trace821_opened = .true.
        end if
        write(821,*) 'GSPEC-A',ispec,Pi,Ti,vol,volnl,x1,htl,spinod(ispec)
C  Now that we have found the volume, we can compute the physical properties and free energy of the species.
	...
	if (htl .eq. 5)  call Ftotsubh(ispec,volnl,Ftot)
        ...
        if (.not. trace822_opened) then
         open(822,file='fort.822',status='unknown',form='formatted')
         trace822_opened = .true.
        end if
        write(822,*) 'GSPEC',ispec,Pi,Ti,vol,volnl,Ftot,Cp,Cv,gamma,K,Ks,alp,ph,ent,deltas,tcal,zeta,Gsh,uth,uto,thet,pzp
c	volve = vol
        gspec = Ftot
