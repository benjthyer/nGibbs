	...
	save Kdiffmin
        logical trace811_opened, trace812_opened, trace813_opened
        save trace811_opened, trace812_opened, trace813_opened
        data trace811_opened /.false./
        data trace812_opened /.false./
        data trace813_opened /.false./
        d = 1.e-6
     ...
     &                    gamma,q,etas,detasdv,qp)
        ...
        xT = thet/Ti
        xo = thet/To
c       Phase-entry checkpoint: thermal inputs after parset/gamset
        if (.not. trace811_opened) then
         open(811,file='fort.811',status='unknown',form='formatted')
         trace811_opened = .true.
        end if
        write(811,*) 'THERM-A',ispec,Ti,To,Vi,Vo,wd1,wd2,wd3,ws1,ws2,ws3,
     &                we1,we2,we3,we4,wou,wol,gamma,q,etas,detasdv,pzp,anh,xT,xo
...
c       Phase-entry checkpoint: thermal-kernel outputs before EoS corrections
        if (.not. trace812_opened) then
         open(812,file='fort.812',status='unknown',form='formatted')
         trace812_opened = .true.
        end if
        write(812,*) 'THERM-B',ispec,Ti,To,uth,uto,Cv,Cvo,zeta,zeto,tcal,anh
c       qp = 0.0
        a1 = 1.5*(Kop - 4.)
        a2 = 0.0
...
c	write(999,'(99f16.6)') 18.015268/Vi,Pi,K,Kdiff,Kdiffmin,pn,kn
        ...
        if (.not. trace813_opened) then
         open(813,file='fort.813',status='unknown',form='formatted')
         trace813_opened = .true.
        end if
        write(813,*) 'THERM',ispec,Ti,Vi,Vo,Cp,Cv,gamma,q,K,Ks,alp,Ftot,ph,ent,deltas,tcal,zeta,Gsh,uth,uto,thet,pzp,Vdeb,gamdeb
