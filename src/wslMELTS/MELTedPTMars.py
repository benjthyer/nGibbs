...
Out_Folder = 'Mars_MELTS'
output_file = 'ChasMELTS'
...
#in_array = grid_sample([[1,10000,50],[1500,1500,1],[-5,5,5]])
#in_array = grid_sample([[1000,10000,3],[1500,1500,1],[-4,4,5]])
...
ox_labels = MarsRocks.columns[1:]
unsortedMars = MarsRocks.iloc[:,1:].to_numpy()
MarsInputTable = np.zeros((unsortedMars.shape[0],len(oxide_dict)))
for i, ox in enumerate(ox_labels):
    if ox != 'MnO':
        MarsInputTable[:,oxide_dict[ox]] = unsortedMars[:,i]
        
#Phase diagram generation

for j, MarsArr in enumerate(MarsInputTable):
    ...
    for batch in np.arange(total_batches).astype(int): # Handle remainder at final batch
        ...
        ensemble_MELTSV2.forward_ensemble(in_array[(batch*batch_size):end], keys, only_phases=allowed_phases, batchname = batchname, 
                                        end = 800, EnsembleLocation=EnsembleLocation, WSL = True, compression=False, delta = -2, fxtal=False)
        ...
        ensemble_MELTSV2.import_MELTS_components(EnsembleLocation=EnsembleLocation, batchname=batchname, fO2Arr=in_array[(batch*batch_size):end,2], dataname = f'Applications/{rockname}_BatchMELTS.csv')
