...
import numpy as np
import torch
#from burnman.constants import logish_eps
from burnman.utils.chemistry import process_solution_chemistry
from burnman import constants
import warnings
from burnman.classes.material import material_property, cached_property
...
class IdealSolution(SolutionModel):
    ...
    # -------------------- PyTorch batched interfaces --------------------
    def batched_excess_partial_entropies(self, temperature, molar_fractions):
        """
        Batched version of excess partial entropies.
        :param temperature: torch tensor shape [B] (unused for ideal part)
        :param molar_fractions: torch tensor shape [B, n_endmembers]
        :returns: torch tensor shape [B, n_endmembers]
        """
        if not torch.is_tensor(molar_fractions):
            raise TypeError("molar_fractions must be a torch tensor for batched interface")

        # site occupancies: [B, n_sites] = [B,n_end] @ [n_end, n_sites]
        end_nocc = torch.as_tensor(self.endmember_noccupancies, device=molar_fractions.device, dtype=molar_fractions.dtype)
        site_mult = torch.as_tensor(self.site_multiplicities, device=molar_fractions.device, dtype=molar_fractions.dtype)

        site_noccupancies = molar_fractions @ end_nocc
        site_multiplicities = molar_fractions @ site_mult

        site_occupancies = site_noccupancies * torch_inverseish(site_multiplicities)

        # lna: [B, n_end] = [n_end, n_sites] (T) @ [B, n_sites]
        lna = torch.einsum("ij,bj->bi", torch.as_tensor(self.endmember_noccupancies, device=molar_fractions.device, dtype=molar_fractions.dtype), (torch_logish(site_noccupancies) - torch_logish(site_multiplicities)))

        normalisation_constants = torch.as_tensor(self.endmember_configurational_entropies / constants.gas_constant, device=molar_fractions.device, dtype=molar_fractions.dtype)
        lna = lna + normalisation_constants[None, :]

        # excess partial entropies: -R * lna
        return -constants.gas_constant * lna

    def batched_excess_partial_gibbs_energies(self, temperature, molar_fractions):
        # G_xs = -T * S_xs
        S_xs = self.batched_excess_partial_entropies(temperature, molar_fractions)
        return -temperature[:, None] * S_xs

    def batched_excess_partial_volumes(self, pressure, temperature, molar_fractions):
        return torch.zeros_like(molar_fractions, device=molar_fractions.device, dtype=molar_fractions.dtype)

    def batched_excess_gibbs_energy(self, pressure, temperature, molar_fractions):
        parts = self.batched_excess_partial_gibbs_energies(temperature, molar_fractions)
        return (molar_fractions * parts).sum(-1)

    def batched_Cp_excess(self, pressure, temperature, molar_fractions):
        """Batched heat capacity excess. For ideal solution, always zero."""
        return torch.zeros(molar_fractions.shape[0], device=molar_fractions.device, dtype=molar_fractions.dtype)

    def batched_VoverKT_excess(self, pressure, temperature, molar_fractions):
        """Batched V/K_T excess. For ideal solution, always zero."""
        return torch.zeros(molar_fractions.shape[0], device=molar_fractions.device, dtype=molar_fractions.dtype)

    def batched_alphaV_excess(self, pressure, temperature, molar_fractions):
        """Batched alphaV excess. For ideal solution, always zero."""
        return torch.zeros(molar_fractions.shape[0], device=molar_fractions.device, dtype=molar_fractions.dtype)
...
class AsymmetricRegularSolution(IdealSolution):
    ...
    # -------------------- PyTorch batched interfaces --------------------
    def batched_phi(self, molar_fractions):
        """Batched phi for molar_fractions tensor [B, n]"""
        alphas = torch.as_tensor(self.alphas, device=molar_fractions.device, dtype=molar_fractions.dtype)
        phi = alphas[None, :] * molar_fractions
        denom = phi.sum(dim=1, keepdim=True)
        return phi / denom

    def batched_non_ideal_interactions(self, W, molar_fractions):
        """Compute non-ideal interactions Wint for batch: returns [B, n_end]"""
        device = molar_fractions.device
        dtype = molar_fractions.dtype
        n = self.n_endmembers
        phi = self.batched_phi(molar_fractions)

        # Convert arrays to tensors
        W_t = torch.as_tensor(W, device=device, dtype=dtype)
        alphas = torch.as_tensor(self.alphas, device=device, dtype=dtype)

        eye = torch.eye(n, device=device, dtype=dtype)[None, :, :]
        ones = torch.ones((1, n, n), device=device, dtype=dtype)

        # q[b,i,j] = delta_ij - phi[b,i]
        q = eye - phi[:, :, None] * ones

        temp = torch.matmul(q, W_t[None, :, :])
        elem = temp * q
        Wint = -alphas[None, :] * elem.sum(dim=-1)
        return Wint

    def batched__non_ideal_excess_partial_gibbs(self, pressure, temperature, molar_fractions):
        Eint = self.batched_non_ideal_interactions(self.We, molar_fractions)
        Sint = self.batched_non_ideal_interactions(self.Ws, molar_fractions)
        Vint = self.batched_non_ideal_interactions(self.Wv, molar_fractions)
        return Eint - temperature[:, None] * Sint + pressure[:, None] * Vint

    def batched_excess_partial_gibbs_energies(self, pressure, temperature, molar_fractions):
        ideal = torch.as_tensor(IdealSolution._ideal_excess_partial_gibbs(self, temperature, molar_fractions), device=molar_fractions.device, dtype=molar_fractions.dtype) if False else None
        # Use IdealSolution batched parts for ideal contribution
        ideal_parts = IdealSolution.batched_excess_partial_gibbs_energies(self, temperature, molar_fractions)
        nonideal = self.batched__non_ideal_excess_partial_gibbs(pressure, temperature, molar_fractions)
        return ideal_parts + nonideal

    def batched_excess_partial_entropies(self, pressure, temperature, molar_fractions):
        ideal_ent = IdealSolution.batched_excess_partial_entropies(self, temperature, molar_fractions)
        nonideal_ent = self.batched_non_ideal_interactions(self.Ws, molar_fractions)
        return ideal_ent + nonideal_ent

    def batched_excess_partial_volumes(self, pressure, temperature, molar_fractions):
        return self.batched_non_ideal_interactions(self.Wv, molar_fractions)

    def batched_excess_gibbs_energy(self, pressure, temperature, molar_fractions):
        parts = self.batched_excess_partial_gibbs_energies(pressure, temperature, molar_fractions)
        return (molar_fractions * parts).sum(-1)

    def batched_Cp_excess(self, pressure, temperature, molar_fractions):
        """Batched heat capacity excess. For regular solutions, typically zero unless explicitly modeled."""
        return torch.zeros(molar_fractions.shape[0], device=molar_fractions.device, dtype=molar_fractions.dtype)

    def batched_VoverKT_excess(self, pressure, temperature, molar_fractions):
        """Batched V/K_T excess. For regular solutions, typically zero unless explicitly modeled."""
        return torch.zeros(molar_fractions.shape[0], device=molar_fractions.device, dtype=molar_fractions.dtype)

    def batched_alphaV_excess(self, pressure, temperature, molar_fractions):
        """Batched alphaV excess. For regular solutions, typically zero unless explicitly modeled."""
        return torch.zeros(molar_fractions.shape[0], device=molar_fractions.device, dtype=molar_fractions.dtype)
