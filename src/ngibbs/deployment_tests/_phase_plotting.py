"""Stacked phase-abundance diagram helpers (deployable copy).

A self-contained copy of ``builder/plotting.py`` so the deployment tests can
draw phase stacks without importing the training-side ``builder`` package
(which is not part of the wheel).
"""
...
def phase_colors() -> dict:
    """{phase_abbreviation: matplotlib_color} for stacked phase diagrams."""
    col = {}
...
def build_ordered_phases(mass_phasedict: dict) -> list:
    """(long_name, abbrev) pairs in IP_PYROLITE order, restricted to mass_phasedict."""
    ordered = []
...
def draw_phase_stack(ax, data: np.ndarray, row_idx: np.ndarray,
    ...
    """Draw a stacked phase-fraction-vs-pressure diagram onto ``ax`` (cleared first)."""
    ax.cla()
...
def flagged_pressure_spans(pres_sorted: np.ndarray, flagged: np.ndarray) -> list:
    """Group a boolean mask (aligned to ascending pres_sorted) into (start, end) spans."""
    spans = []
    start = None
